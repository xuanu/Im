...
public class RecordButton extends Button {
    ...
    public void setTooShort(String pTooShort) {
        mTooShort = pTooShort;
    }
    ...
    private void setDefaultFilePath() {
        File file2 = new File(getContext().getExternalFilesDir("audio"), (!TextUtils.isEmpty(mPrefix) ? mPrefix : "default") + "_" + System.currentTimeMillis() + ".mp3");
    ...
    private void finishRecord() {
        ...
        if (intervalTime < MIN_INTERVAL_TIME) {
            if (!TextUtils.isEmpty(mTooShort)) {
                Toast.makeText(getContext(), mTooShort, Toast.LENGTH_SHORT).show();
            } else {
        ...
        if (finishedListener != null)
            finishedListener.onFinishedRecord(mFilePath, Math.round(intervalTime));
    }

    private void cancelRecord() {
        if (mRecorder == null || mthread == null) return;
        stopRecording();
        mDialog.dismiss();
        File file = new File(mFilePath);
        if (file.exists())
